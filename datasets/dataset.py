...
def _get_classes(data_subset_dir_path):
  ...
  for filename in os.listdir(data_subset_dir_path):
    if path.isdir(path.join(data_subset_dir_path, filename)):
      class_names.append(filename)

  return sorted(class_names)


def _get_filepaths(data_subset_path, split_name):
  ...
  class_names = os.listdir(data_subset_path)

  for class_name in class_names:
    data_subset_class_path = os.path.join(data_subset_path, class_name)
    if os.path.isdir(data_subset_class_path):
      for image_name in os.listdir(data_subset_class_path):
        image_path = path.join(data_subset_class_path, image_name)
  ...
  for video_frame_dir_path in video_frame_dirs:
    ...
    if path.isdir(video_frame_dir_path):
      ...
      for class_sub_dir_name, class_sub_dir_path in class_sub_dir_paths.items():
        ...
        if create_eval_subset:
          tf.logging.info('eval n_frames: {}'.format(n_frames))
          tf.logging.info('eval frame_set len: {}'.format(len(frame_set)))
