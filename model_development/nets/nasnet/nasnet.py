...
def cifar_config(is_training=True):
...
def large_imagenet_config(is_training=True):
...
# label smoothing: 0.1
# clip global norm of all gradients by 10
def _15_at_1920_imagenet_config(is_training=True):
...
# learning rate scaling factor: 0.8
# num epochs per decay: 2
# sync sgd with 50 replicas
# auxiliary head weighting: 0.4
# label smoothing: 0.1
# clip global norm of all gradients by 10
def _7_at_1920_imagenet_config(is_training=True):
  drop_path_keep_prob = 1.0 if not is_training else 0.8
  return tf.contrib.training.HParams(
      stem_multiplier=3.0,
      dense_dropout_keep_prob=0.5,
...
def build_nasnet_cifar(images,
  ...
  hparams = cifar_config(is_training=is_training) if config is None else copy.deepcopy(config)
...
def build_nasnet_15_at_1920(images, num_classes,
                       is_training=True,
                       final_endpoint=None):
  """Build NASNet Large model for the ImageNet Dataset."""
  hparams = _15_at_1920_imagenet_config(is_training=is_training)
  ...
  with arg_scope([slim.dropout, nasnet_utils.drop_path, slim.batch_norm],
    ...
    with arg_scope([slim.avg_pool2d,
      ...
      return _build_nasnet_base(images,
                                ...
                                stem_type='imagenet',
                                final_endpoint=final_endpoint)
build_nasnet_15_at_1920.default_image_size = 299


def build_nasnet_7_at_1920(images, num_classes,
                       is_training=True,
                       final_endpoint=None):
  """Build NASNet Large model for the ImageNet Dataset."""
  hparams = _7_at_1920_imagenet_config(is_training=is_training)
  ...
  with arg_scope([slim.dropout, nasnet_utils.drop_path, slim.batch_norm],
    ...
    with arg_scope([slim.avg_pool2d,
      ...
      return _build_nasnet_base(images,
                                ...
                                stem_type='imagenet',
                                final_endpoint=final_endpoint)
build_nasnet_7_at_1920.default_image_size = 299
...
def build_nasnet_large(images,
                       ...
                       config=None):
  """Build NASNet Large model for the ImageNet Dataset."""
  hparams = (large_imagenet_config(is_training=is_training) if config is None
             else copy.deepcopy(config))
  _update_hparams(hparams, is_training)
