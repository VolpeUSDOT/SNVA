...
tf.app.flags.DEFINE_float(
  ...
  'The ratio of total memory across all available GPUs to use with this '
  'process. Defaults to a suggested max of 0.9.')

tf.app.flags.DEFINE_integer(
  'gpu_device_num', 0,
  'The device number of a single GPU to use for evaluation on a multi-GPU '
...
def interrupt_handler(signal_number, _):
  tf.logging.info('Received interrupt signal (%d). Unsetting '
                  'CUDA_VISIBLE_DEVICES environment variable.', signal_number)
...
def main(_):
  ...
  if FLAGS.cpu_only:
    ...
    if FLAGS.num_clones == 1:
      tf.logging.info(
        'Setting CUDA_VISIBLE_DEVICES environment variable to %d.',
        FLAGS.gpu_device_num)
      os.putenv('CUDA_VISIBLE_DEVICES', str(FLAGS.gpu_device_num))

      signal.signal(signal.SIGINT, interrupt_handler)

  with tf.Graph().as_default() if FLAGS.num_clones > 1 else \
           tf.Graph().as_default(), tf.device(
      device_name):
    #######################
    # Config model_deploy #
    #######################
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      if preprocessing_name == 'standardization':
        if FLAGS.standardization_dataset_name and \
